...
vector<pdd> input, vertex;
vector<pii> edge, area;
vector<pair<pair<double, double>, int>> ordering;

ostream& operator<<(ostream& os, const pdd& p){
    os << p.first << " " << p.second << '\n';
    return os;
}
...
int main(){
    fastio;
    int n; cin >> n;
    ordering.resize(n); input.resize(n);
    for(int i = 0; i < n; i++){
        cin >> ordering[i].first.second >> ordering[i].first.first;
        input[i] = {ordering[i].first.second, ordering[i].first.first};
        ordering[i].second = i;
    }
    ...
    cout << "_\n";
    for (pii point: area){
        point.first = ordering[point.first].second;
        point.second = ordering[point.second].second;
        cout << point;
    }
}
